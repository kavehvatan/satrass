...
// مسیر داده‌ها (طبق ساختاری که قبلاً استفاده می‌کردیم)
import vendors from "../data/vendors";          // لیست وندورهای «تجهیزات»
import services from "../data/services.json";   // آیتم‌های «خدمات»

/* ==== رنگ‌های برند (طبق سایت) ==== */
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="ارائه راهکارها و نرم‌افزارها، تجهیزات سازمانی، و خدمات تخصصی توسط ساتراس." />
      </Head>
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="rounded-3xl bg-white/60 dark:bg-slate-900/40 backdrop-blur-md ring-1 ring-slate-200/60 dark:ring-slate-700/50 p-8 md:p-12 flex flex-col md:flex-row gap-8 items-center">
            <div className="flex-1 text-center md:text-right">
              ...
              <div className="mt-6 flex flex-wrap justify-center md:justify-start gap-3">
                ...
                <Link
                  href="/contact"
                  className="btn-ghost" // همون استایل قبلیِ دکمه ثانویه‌تون
                >
                  ارائه مشاوره
                </Link>
              </div>
        ...
        <section className="mb-14">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link href="/products" className="text-sm font-bold text-slate-700 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white">
              مشاهده همه
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {vendors?.map((v) => {
              ...
              // تصویر پس‌زمینهٔ خیلی کمرنگ (fallback) – قبلاً اضافه شده بود،
              // تغییری نمی‌دهیم تا چیزی دیگه خراب نشه.
              ...
              return (
                ...
                >
                  ...
                  <div className="relative flex flex-col items-center justify-center p-6 h-28">
                    ...
                    {/* طبق خواستهٔ اخیر: فقط لوگو نمایش داده شود؛ اسم برند را ننویسیم */}
                  </div>
                </Link>
              );
            })}
        ...
        {/* ============== خدمات (با پس‌زمینهٔ «کامل و یک‌رنگ») ============== */}
        <section className="mb-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {services?.items?.map((s, i) => {
              const bg = serviceSolidBg(i); // یکی از دو رنگ برند
              const isLight = bg === BRAND_AMBER; // روی کهربایی، متن تیره‌تر خواناتر است
              ...
              return (
                ...
                >
                  {/* اگر آیکون/لوگو برای سرویس داشتیم، کنار عنوان می‌آوریم؛
                      تغییری در داده‌ها نمی‌دهیم تا چیز دیگری به‌هم نخورد */}
                  <div className="flex items-center gap-3 mb-3">
                    {s.icon && (
                      <span
                        ...
                        aria-hidden
                      >
                        {/* s.icon می‌تواند مسیر تصویر باشد */}
                  ...
                  {s.desc && (
                    <p className={`leading-7 ${bodyColor}`}>
                      {s.desc}
                    </p>
                  )}

                  {/* اگر لینک ادامه برای هر سرویس تعریف شده بود */}
                  {s.href && (
                    <div className="mt-5">
                      <Link
                        href={s.href}
